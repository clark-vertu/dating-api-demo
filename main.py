...
async def voice_chat(file: UploadFile = File(...)) -> Dict[str, str]:
    """
    接收一个音频文件（wav/m4a 等），
    ...
    # ---------- 1. STT：whisper-large-v3 ----------
    # 参考官方文档，通过 Groq SDK 调用 audio.transcriptions.create :contentReference[oaicite:2]{index=2}
    try:
    ...
    # ---------- 3. TTS：playai-tts ----------
    # 官方示例：client.audio.speech.create(...).write_to_file("speech.wav") :contentReference[oaicite:3]{index=3}
    try:
